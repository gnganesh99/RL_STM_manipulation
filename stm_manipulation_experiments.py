...
def manipulation_and_scan_tcp(initial_coords, final_coords, manipulation_params, default_params, 
                              ...
                              drift_compensation = False, target_reached = False, get_tiprec_buffer = True):
    ...
    while not m_done:
        ...
        try:
            ...
            m_done, tiprec_data = e1.manipulation(initial_coords = initial_coords[0], final_coords=final_coords[0], manipulation_params=modified_action_params, 
                           default_params = modified_default_params, bias_slew_rate=0.033, setpoint_slew_rate=80E-9, get_buffer=get_tiprec_buffer)
        except:
            pass
    ...
    return s_done, tiprec_data




def path_manipulation_scan_tcp(path_coords, manipulation_params, default_params,   # need to introduce buffer reading!
                              sxm_dir, scan_params, drift, expt_name,
                              drift_compensation = False, target_reached = False, 
                              get_tiprec_buffer = True, drift_tolerance = 2.5):
    tiprec_data =  None
    if target_reached:
        
        print("\nTarget reached !!!\n")
        return True, tiprec_data
    ...
    while not m_done:
        ...
        try:
            ...
            # point coords are converted from 2D to 1D for manipulation function.
            m_done, tiprec_data = e1.path_manipulation(path_coords, manipulation_params=modified_action_params, 
                           default_params = modified_default_params, bias_slew_rate=0.033, setpoint_slew_rate=90E-9, 
                           get_buffer=get_tiprec_buffer)
        except:
            pass
    ...
    if drift_compensation:
        ...
        # Add the drift to the tip position. This is because tip /frame direction is opposite. if the tip drifts in -x, objects drift in x.
        center_drift = np.asarray([frame_size/2, frame_size/2]) + frame_drift
        ...
        if len(drift_log) > 1:
            n_drift_log = len(drift_log) 
            start_avg =  n_drift_log - min(n_drift_log, 10) #average over last 10
            avg_x_drift = np.mean(abs(drift_log[start_avg:n_drift_log, 0]))
            avg_y_drift = np.mean(abs(drift_log[start_avg:n_drift_log, 1]))
        ...
        # if drift is < 1.2 * average drift, then compensate drift. else skip large drift compensation, possibly due to disturbance.
        if abs(delta_x) < drift_tolerance*avg_x_drift and abs(delta_y) < drift_tolerance*avg_y_drift:            
...
def do_manipulation_LV_dummy(initial_coords, final_coords, default_action_params, action_params, sxm_dir):
    ...
    origin_dir = r"C:\Users\ggn\1_py_scripts\Manipulation\Manipulation_part2\dummy_manipulation\sxm_dir_3\source_folder"
