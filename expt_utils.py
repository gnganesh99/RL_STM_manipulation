...
import time
import matplotlib
#matplotlib.use("Agg")   # Important: non-interactive backend, safe for threads
...
def save_scan_img(img_path, image_array):
    ...
    Thread-safe image saver using Matplotlib's OO API using Agg backend
    """
    ...
    ax.set_axis_off()
    im = ax.imshow(image_array, origin='lower', aspect='equal')
    fig.savefig(img_path, bbox_inches='tight', pad_inches=0)
    plt.close(fig)  # crucial: close figure to free resources
...
def saved_next_sxmfilenames(expt_dir, basename, index_len = 4):
    ...
    """
    Outputs the saved filename, next-to-be filename and next number based on the basename

    i/p:
        expt_dir: directory of the experiment with sxm files
        basename: base filename string
        index_len: length of the numerical suffix, default = 4
    O/p:
        saved_filename: last saved filename with the highest numerical suffix
        next_filename: next filename to be saved
        next_num: next numerical suffix (int)
    """
    ...
    def basename_exists(basename, filename, index_len = 4):
        ...
        """
        Checks if the given basename exists in the filename with the correct numerical index length
        i/p:
            basename: string
            filename: string
            index_len: length of the numerical suffix, default = 4
