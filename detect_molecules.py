# -*- coding: utf-8 -*-
"""
Functions for the detection of molecules using YOLO object detection model.
...
from yolo_param_tuning import tune_conf_iou_for_image

trained_model = r"C:\Users\Administrator\Py_Scripts_ganesh\manipulation_scripts_3\yolo_models\train69\weights\best.pt"   #train69/best.pt


def predict_labels(img_path, detect_dict = None, optimize_yolo_params = True):
    
    """
    Predicts and outputs the object labels in the image.
    The conf and iou parameters of the yolo-prediction can be optimized based on the target_n (number of detections) value in the detect_dict

    I/p:
        img_path: image path of the image on which the yolo-prediction is computed
        detect_dict: dictionary containing the target_n value for optimization.
            target_n: number of detections to optimize for
            use_prev: boolean to use previous image labels to correlate spatial positions.
        optimize_yolo_params: True/False to optimize the conf and iou parameters based on
    ...
    target_n  = detect_dict.get('target_n', None) if detect_dict is not None else None

    if detect_dict is None or optimize_yolo_params == False or target_n is None or target_n < 1:  # target_n==0 means no optimization. can be user controlled.
        result = model_test.predict(source = img_path, save = True, save_txt =  True, iou = 0.1, conf = 0.2)
    else:
        best_param, best_loss = tune_conf_iou_for_image(model_test, img_path, label_dict=detect_dict)
        best_iou = best_param.get('iou', 0.1)
        best_conf = best_param.get('conf', 0.25)
        clear_output(wait = True)
        result = model_test.predict(source = img_path, save = True, save_txt =  True, iou = best_iou, conf = best_conf)
