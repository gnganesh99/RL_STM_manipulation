...
from rrt_star_algo import run_rrt_star
...
def get_manipulation_coords(sxm_filepath, img_dir, X_target, max_len = False):
    ...
    if label_path != 'no_detections':
        ...
        #save_path_img(sxm_dir, file_name, labels, bw, bh, initial_coords, final_coords, bw_pos, bh_pos)
    ...
    return initial_coords, final_coords, scan_params, corrected_position, drift, labels_exist


def get_manipulation_coords_MO(sxm_filepath, img_dir, X_target, iteration, obj_idx, label_margin = 0.0, anchor_tuple = None, detect_dict = None):
    ...
    anchor_idx = anchor_tuple[1] if anchor_tuple is not None else 0


    if prediction_logged == False:
        labels, bw, bh, label_path = predict_labels(img_path, detect_dict=detect_dict)
    else:
        labels, bw, bh, label_path = get_predicted_labels(img_path)
    ...
    if label_path != 'no_detections':
        ...
        if iteration == 0:
            marker_labels = X_target

        else:
            #assign objects to previous object positions
            marker_labels = read_log("markers_log.txt")

            #assign current object to target position
            marker_labels[obj_idx] = X_target[obj_idx]
            
            
            # if anchor_palaced, consider the anchor target for assignment. this helps if anchor molecule is disturbed.
            if anchor_tuple is not None:
                anchor_placed, anchor_idx = anchor_tuple
                
                if anchor_placed:
                   marker_labels[anchor_idx] = X_target[anchor_idx]
        ...
        # include only the labels in margin.

        label_margin = min((np.mean(bw)+np.mean(bh))/4, label_margin)
        mask = np.array([point_within_margin(l, margin=label_margin) for l in labels])

        labels, bw, bh = labels[mask], bw[mask], bh[mask]
        all_labels = [labels, bw, bh]


        initial_coords, marker_coords, bw_pos, bh_pos = linear_assignment(labels, bw, bh, marker_labels, anchor_tuple = anchor_tuple)
        #initial_coords, marker_coords, bw_pos, bh_pos = linear_assignment_by_index(labels, bw, bh, marker_labels)
        label_width = (np.mean(bw_pos)+np.mean(bh_pos))/2
                

        write_anchor_log("markers_log.txt", initial_coords)

        #save the intial positions w.ro.to the filename

        if iteration == 0:
            final_coords = marker_coords
        ...
        # Save assigned image
        save_assigned_img(sxm_dir, file_name, initial_coords, final_coords, bw_pos, bh_pos)
        #save_path_img(sxm_dir, file_name, labels, bw, bh, initial_coords, final_coords, bw_pos, bh_pos, obj_idx)


        # Include drift correction!
        current_initial_coord =  np.asarray([initial_coords[obj_idx]])
        current_final_coord = np.asarray([final_coords[obj_idx]])
        
        #drift, corrected_position = no_drift_correction(sxm_filepath, img_dir, current_initial_coord, current_final_coord)
        drift, corrected_position = drift_correction_anchor_label(sxm_filepath, img_dir, current_initial_coord, X_target, anchor_idx=anchor_idx)
    

    return initial_coords, final_coords, scan_params, corrected_position, drift, labels_exist, label_width, all_labels
    ...
    for object_pos in labels:
        ...
        if (object_pos[0] != initial_coord[0]) and (object_pos[1] != initial_coord[1]):
            ...
            for point in mani_path:
                ...
                if distance(point, object_pos) < 1.5*avg_bw:
...
def linear_assignment(x_current, bw, bh, x_target, anchor_tuple = None) -> np.ndarray:
    ...
    '''
    # Work on copies to avoid surprising the caller
    x_current = np.asarray(x_current).copy()
    x_target  = np.asarray(x_target).copy()
    bw        = np.asarray(bw).copy()
    bh        = np.asarray(bh).copy()
    ...
    if anchor_placed:
        anchor_target = np.asarray([x_target[anchor_idx]])
        cost_1 = distance_cost_matrix(x_current, anchor_target)
        row_1, col_1, sorted_costs_1 = sort_assignment_col(cost_1)
        anchor_obj = x_current[row_1[0]]
        bw_anchor = bw[row_1[0]]
        bh_anchor = bh[row_1[0]]

        x_current = np.delete(x_current, row_1[0], axis = 0)
        bw = np.delete(bw, row_1[0], axis = 0)
        bh = np.delete(bh, row_1[0], axis = 0)
        x_target = np.delete(x_target, anchor_idx, axis = 0)
    ...
    if len(x_target) > 0:
        ...
        initial_atoms = np.empty((0,2))
        final_atoms = np.empty((0,2))
        bw_atom = np.empty((0,))
        bh_atom = np.empty((0,))
    

    if anchor_placed:
        initial_atoms = np.insert(initial_atoms, anchor_idx, anchor_obj, axis=0)
        final_atoms = np.insert(final_atoms, anchor_idx, anchor_target[0], axis=0)
        bw_atom = np.insert(bw_atom, anchor_idx, bw_anchor, axis=0)
        bh_atom = np.insert(bh_atom, anchor_idx, bh_anchor, axis=0)
...
def linear_assignment_by_index(x_current, bw, bh, x_target):
    ...
    for i in range(len(x_target)):
        cost = distance_cost_matrix(x_current, x_target[i])
        row, col, sorted_costs = sort_assignment_col(cost)    
        initial_atoms.append(x_current[row])
        bw_atom = bw[row]
        bh_atom = bh[row]

        np.delete(x_current, row)
...
def save_sxm_image(sxm_filepath, img_dir):
    ...
    start = time.time()
    while True:
        if os.path.exists(sxm_filepath):
            break
        if time.time()-start > 10:
            print("sxm path does not exist")
            break
...
def no_drift_correction(sxm_filepath, img_dir, X_initial, X_final = None):
